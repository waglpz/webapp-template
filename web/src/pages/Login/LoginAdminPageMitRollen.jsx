...
export function LoginAdminPageMitRollen() {
  ...
  const { rolesList, orgunitsList } = useSelector(miscReduxData);
  ...
  const submitCode = (currentToken) => {
    const handleToken = (_token) => {
      const tokenData = decodeToken(_token);
      tokenData && dispatch(login({
        ...tokenData.userData,
        authData: {
          ...tokenData,
          token: _token,
  ...
  const onSelect = (action) => {
    ...
    const actions = {
      roles: (event) => {
        setRoles(fix(event.target.value));
      },
      orgunits: (event) => {
        setOrgunits(fix(event.target.value));
      },
    };
          ...
          <CardContent>
            <>
              ...
              <FormControl fullWidth sx={{ marginTop: 3 }}>
                <InputLabel
                  id="admin-login-select-role-label">Rolle</InputLabel>
                <Select
                  multiple
                  ...
                  value={roles}
                  label="Rolle"
                  onChange={onSelect('roles')}
                >
                  <MenuItem value="" disabled></MenuItem>
                  {
                    Object.values(rolesList).map((r) => {
                      return <MenuItem key={r.value}
                                       value={r.value}
              ...
              <FormControl fullWidth sx={{ marginTop: 3 }}>
                <InputLabel
                  id="admin-login-select-orgunit-label">Orgunit</InputLabel>
                <Select
                  multiple
                  labelId="admin-login-select-orgunit-label"
                  id="admin-login-select-role"
                  value={orgunits}
                  label="Orgunit"
                  onChange={onSelect('orgunits')}
                >
                  <MenuItem value="" disabled></MenuItem>
                  {
                    Object.values(orgunitsList).map((o) => {
              ...
              <FormControl fullWidth sx={{
                ...
                <Button
                  onClick={submitCode(token)}
                  disabled={requestInProgress || roles.length < 1}
                  sx={{
                    p: .7,
                    pl: 1.5,
                    pr: 1.5,
                ...
                >{useTokenOnly && !token ? 'JWT laden' : 'Weiter mit selected Rolle'}
                </Button>
              </FormControl>
            </>
          </CardContent>
